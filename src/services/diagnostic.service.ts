  ...
  for (const regle of regles) {
    ...
    switch (regle.code) {
      ...
      case "JS-004": {
        ...
        let matchDecl: RegExpExecArray | null;

        while ((matchDecl = declarationRegex.exec(code)) !== null) {
          if (matchDecl[1]) declarations.add(matchDecl[1]);
        }

        const paramsRegex = /function\s*\w*\s*\(([^)]*)\)/g;
        let matchParams: RegExpExecArray | null;
        ...
        while ((matchParams = paramsRegex.exec(code)) !== null) {
          ...
          if (params) {
            params
              ...
              .filter(Boolean)
              .forEach((p) => declarations.add(p as string));
          }
        }

        const utilisationRegex =
          /console\.log\(\s*([A-Za-z_$][\w$]*)\s*\)/g;

        let matchUsage: RegExpExecArray | null;
      ...
      case "JS-007": {
        ...
        while ((match = constDeclarationRegex.exec(code)) !== null) {
          ...
          const apresDeclaration = code.slice(
            match.index + match[0].length
          );

          const reassignation = new RegExp(
            `\\b${nom}\\s*=(?!=)`
          ).test(apresDeclaration);
       ...
       */
      case "JS-008": {
        const comparaisonNullRegex =
          /\b([A-Za-z_$][\w$]*)\s*(?:===|==)\s*(?:null|undefined)\b/g;

        const variablesNullables = new Set<string>();

        let matchNull: RegExpExecArray | null;

        while ((matchNull = comparaisonNullRegex.exec(code)) !== null) {
          if (matchNull[1]) variablesNullables.add(matchNull[1]);
        }

        for (const nom of variablesNullables) {
          const accesDirect = new RegExp(
            `\\b${nom}\\.[A-Za-z_$]`
          ).test(code);

          const accesOptionnel = new RegExp(
            `\\b${nom}\\?\\.`
          ).test(code);
       ...
       */
      case "JS-013": {
        const fonctionsRegex =
          /\bfunction\s+([A-Za-z_$][\w$]*)\s*\(([^)]*)\)/g;

        let matchFn: RegExpExecArray | null;

        while ((matchFn = fonctionsRegex.exec(code)) !== null) {
          const nom = matchFn[1];
          const params = matchFn[2];

          if (!nom || !params?.trim()) continue;

          const aDesParametresSansDefaut = params
            .split(",")
            .some((p) => !p.includes("="));

          if (!aDesParametresSansDefaut) continue;

          const appelSansArgument = new RegExp(
            `\\b${nom}\\s*\\(\\s*\\)`
          ).test(code);
      ...
      case "JS-016":
        correspond =
          /\b(?:const|let)\s+[A-Za-z_$][\w$]*\s*=\s*[A-Za-z_$][\w$]*\.forEach\s*\(/.test(
            code
          );
        break;
      ...
      case "JS-017":
        correspond =
          ...
          !/\b(?:const|let|var)\s+[A-Za-z_$][\w$]*\s*=\s*[A-Za-z_$][\w$]*\.map\s*\(/.test(
            code
          ) &&
      ...
      case "JS-018": {
        ...
        while ((matchIndex = accesIndexRegex.exec(code)) !== null) {
          ...
          const utiliseSansVerif =
            /^\s*\.\s*[A-Za-z_$]/.test(suite) &&
            !/\?\./.test(suite);
      ...
      case "JS-019":
        correspond =
          /\b(?:const|let)\s+[A-Za-z_$][\w$]*\s*=\s*(obtenir|charger|recuperer|fetch)[A-Za-z_$]*\s*\([^)]*\)\s*;/.test(
            code
          ) && !/\bawait\b/.test(code);
        break;
      ...
      case "JS-020":
        correspond =
          /^\s*(obtenir|charger|recuperer|creer|supprimer|envoyer)[A-Za-z_$]*\s*\([^)]*\)\s*;\s*$/m.test(
            code
          ) &&
          !/\bawait\b/.test(code) &&
          !/\.then\s*\(/.test(code) &&
      ...
      case "JS-026": {
        const whileRegex =
          /\bwhile\s*\(\s*([A-Za-z_$][\w$]*)\s*[<>]=?\s*[^)]+\)\s*\{([\s\S]*?)\}/g;

        let matchWhile: RegExpExecArray | null;

        while ((matchWhile = whileRegex.exec(code)) !== null) {
          const variable = matchWhile[1];
          const corps = matchWhile[2];

          if (!variable || corps === undefined) continue;

          const estIncrementee = new RegExp(
            `\\b${variable}\\s*(?:\\+\\+|--|\\+=|-=|=[^=])`
          ).test(corps);
       ...
       */
      case "JS-027": {
        const boucleForOfRegex =
          /\bfor\s*\(\s*const\s+[A-Za-z_$][\w$]*\s+of\s+[^)]+\)\s*\{([\s\S]*?)\}/g;

        let matchBoucle: RegExpExecArray | null;
      ...
      case "JS-029": {
        ...
        while ((matchDecl = declarationRegex.exec(code)) !== null) {
          ...
          const utilisee = new RegExp(`\\b${nom}\\b`).test(
            apresDeclaration
          );
      ...
      case "JS-032": {
        ...
        let matchNombre: RegExpExecArray | null;
...
//----------------------------------------------------------------------
      ...
      case "TS-003":
        correspond =
          ...
          !/\bfunction\s+[A-Za-z_$][\w$]*\s*\([^)]*\)\s*:\s*[A-Za-z_$]/.test(
            code
          );
        break;
      ...
      case "TS-005":
        correspond =
          /\b(data|response|payload|body|reponse|donnees)\s*:\s*any\b/i.test(
            code
          );
        break;
      ...
      case "TS-008": {
        ...
        while ((matchType = unionTypeRegex.exec(code)) !== null) {
          ...
          if (!nomType || !litteraux) continue;

          const valeursAutorisees = [
            ...litteraux.matchAll(/"([^"]+)"/g),
          ].map((m) => m[1]);

          const utilisationRegex = new RegExp(
            `:\\s*${nomType}\\s*=\\s*"([^"]+)"`
          );
      ...
      case "REACT-001": {
        const useStateRegex =
          /const\s*\[\s*([A-Za-z_$][\w$]*)\s*,\s*set[A-Z][A-Za-z0-9_$]*\s*\]\s*=\s*useState/g;

        let matchState: RegExpExecArray | null;
        ...
        while ((matchState = useStateRegex.exec(code)) !== null) {
          ...
          const reassignationRegex = new RegExp(
            `\\b${nomState}\\s*=(?!=)`
          ).test(code);
      ...
      case "REACT-003": {
        ...
        while ((matchState = useStateRegex.exec(code)) !== null) {
          ...
          const mutationPropriete = new RegExp(
            `\\b${nomState}\\.[A-Za-z_$][\\w$]*\\s*=(?!=)`
          ).test(code);
      ...
      case "REACT-006": {
        ...
        const hookRegex =
          /\b(useState|useEffect|useMemo|useCallback|useRef|useContext)\s*\(/g;
      ...
      case "REACT-008":
        correspond =
          /\.map\s*\(\s*\([^,)]+,\s*index\s*\)\s*=>[\s\S]*?key\s*=\s*\{?\s*index\s*\}?/.test(
            code
          );
        break;
      ...
      case "REACT-010": {
        ...
        for (const effet of effets) {
          ...
          if (!corps || dependances === undefined) continue;
          ...
          const modifieUneDependance = deps.some((dep) =>
            new RegExp(`\\bset${dep.charAt(0).toUpperCase()}${dep.slice(1)}\\s*\\(`).test(
              corps
            )
          );
       ...
       */
      case "REACT-011": {
        const effetsAvecRessource = code.match(
          /useEffect\s*\(\s*\(\s*\)\s*=>\s*\{[\s\S]*?\}\s*,/g
        );

        if (effetsAvecRessource) {
          correspond = effetsAvecRessource.some(
            (effet) =>
              /(addEventListener|setInterval|setTimeout|subscribe|new WebSocket)/.test(
                effet
              ) &&
              !/(removeEventListener|clearInterval|clearTimeout|unsubscribe|\.close\s*\()/.test(
                effet
              )
          );
        }
      ...
      case "REACT-012": {
        const setterRegex =
          /\bset([A-Z][A-Za-z0-9_$]*)\s*\(\s*([A-Za-z_$][\w$]*)\s*\+/g;
      ...
      case "REACT-013":
        correspond =
          /useEffect\s*\(\s*\(\s*\)\s*=>\s*\{[\s\S]*?\}\s*,\s*\[\s*(envoye|submitted|clicked|clique|valide)\s*\]\s*\)/i.test(
            code
          );
        break;  
        //----------------------------------------
...
export async function verifierCatalogueDiagnostic() {
  ...
  for (const regle of CODE_DOCTOR_RULES) {
