...
exports.verifierEtConsommerQuotaIA = verifierEtConsommerQuotaIA;
exports.enregistrerTokensIA = enregistrerTokensIA;
exports.obtenirEtatQuotaIA = obtenirEtatQuotaIA;
...
async function verifierEtConsommerQuotaIA(utilisateurId) {
    ...
    if (abonnement) {
        console.log("PLAN PREMIUM :", abonnement.plan === "PREMIUM");
        console.log("STATUT ACTIVE :", abonnement.statut.trim() === "ACTIVE");
        console.log("DATE FUTURE :", abonnement.dateRenouvellement > new Date());
    }
    else {
     ...
     */
    await base_1.prisma.aIUsage.upsert({
        where: {
            userId_dateJour: {
                userId: utilisateurId,
                dateJour,
     ...
     * son quota.
     */
    const resultat = await base_1.prisma.aIUsage.updateMany({
        where: {
            userId: utilisateurId,
            dateJour,
            requetes: {
    ...
    if (resultat.count === 0) {
        const usage = await base_1.prisma.aIUsage.findUnique({
            where: {
                userId_dateJour: {
                    userId: utilisateurId,
                    dateJour,
        ...
        throw new Error(`QUOTA_IA_ATTEINT:${plan}:${usage?.requetes ?? limite}:${limite}`);
    }
    const usage = await base_1.prisma.aIUsage.findUnique({
        where: {
            userId_dateJour: {
...
async function enregistrerTokensIA(utilisateurId, dateJour, tokens) {
    if (!Number.isFinite(tokens) || tokens <= 0) {
        return;
    }
    await base_1.prisma.aIUsage.update({
        where: {
            userId_dateJour: {
                userId: utilisateurId,
                dateJour,
...
async function obtenirEtatQuotaIA(utilisateurId) {
    const dateJour = obtenirDateJour();
    const utilisateur = await base_1.prisma.user.findUnique({
        where: {
            id: utilisateurId,
